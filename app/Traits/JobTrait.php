...
trait JobTrait
{
	...
	/******************************************/
	/******************************************/
	public function revalidateMail() {
		$company = Auth::guard('company')->user();
		UserVerification::generate($company);
		UserVerification::send($company, 'User Verification', config('mail.recieve_to.address'), config('mail.recieve_to.name'));
	...
	public function createFrontJob()
    {
		...
		// dd($company);
		// exit;
		if($company->industry_id == 0 || $company->ownership_type_id == 0 || $company->description == null || $company->ceo == null || $company->description == null || $company->description == "" || $company->location == null 
			|| $company->no_of_offices == null || $company->website == null || $company->no_of_employees == null || $company->established_in == null
			|| $company->fax == null || $company->phone == null || $company->country_id == 0 || $company->state_id == 0 || $company->city_id == 0
		) {
			flash(__('Please update the company profile before you can post a job.'))->error();
			return \Redirect::route('company.profile');
		}
		if($company->verified == 0) {
			$this->revalidateMail();
			flash(__('Please verify your email, before you can post a job.'))->error();
			return \Redirect::route('company.home');
		}
		if($company->is_active == 0) {
			flash(__('Your account isn\'t active. Please wait while we verify your details. You can write to '.config('mail.recieve_to.address'). ' in case you want to expedite the process.'))->error();
			return \Redirect::route('company.home');
		}		
		if((bool)config('employer.is_employer_package_active')){
		if(
